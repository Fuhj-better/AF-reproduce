...
########################################################################
    ...
    def _get_senslist(self,node_senslist):
        ...
        for sens in node_senslist.list:
            edge=sens.type
            sig = None if sens.sig is None else sens.sig.name
...
# 针对 Input, Output, Inout 节点进行收集
    ...
    def get_rtlinfo(self):
        return self.collected_info

# def parse_rtl(root_dir):
#     rtl_dir=os.path.join(root_dir,'rtl')
#     if os.path.exists(rtl_dir) is False:
#         print(f"{rtl_dir} in {root_dir} does not exit.")
#         sys.exit(-1)
#     in_file_list=glob(os.path.join(rtl_dir, "*.v"))
#     pre_dir=os.path.join(root_dir,'pre_rtl')
