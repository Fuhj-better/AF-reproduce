...
class KGBuilder:
    ...
    def _build_wire(self, filepath, wire):
        ...
        parent_module_name=wire.get('parent_module','unknown')
    ...
    def _to_parquet(self,root_dir,entities_json,relationships_json):
        rtl_parquet_dir=os.path.join(root_dir,'rtl_parquet')
        os.makedirs(rtl_parquet_dir,exist_ok=True)
        entities_parquet_file=os.path.join(rtl_parquet_dir,'entities.parquet')
        relationships_parquet_file=os.path.join(rtl_parquet_dir,'relationships.parquet')
        try:
            df_e = pd.DataFrame(entities_json)
            df_r = pd.DataFrame(relationships_json)
            
            df_e.to_parquet(entities_parquet_file,index=False)
            df_r.to_parquet(relationships_parquet_file,index=False)

            print("json to parquet done")
        except Exception as e:
            print(f"json to parquet error:{e}")


    def get_kg(self,root_dir):
        self._build_kg()
        entities_json=self._to_json(self.entities)
        relationships_json=self._to_json(self.relationships)
        # self._to_parquet(root_dir,entities_json,relationships_json)
        self._to_parquet(root_dir,self.entities,self.relationships)
        return entities_json,relationships_json 
